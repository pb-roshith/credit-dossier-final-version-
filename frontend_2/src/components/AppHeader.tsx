...
import { BookOpen, Factory, LayoutDashboard, FilePlus2, LogOut, UserRound } from "lucide-react";
    ...
    <header className="border-b bg-primary text-primary-foreground">
      <div className="mx-auto flex max-w-[1400px] flex-wrap items-center justify-between gap-2 px-3 py-2 sm:px-6">
        ...
        <div className="flex items-center gap-3">
          <Link to="/profile" className="rounded-md px-2 py-1 text-right text-xs leading-tight hover:bg-primary-foreground/10">
            <div className="flex items-center justify-end gap-1 font-semibold">
              <UserRound className="h-3.5 w-3.5" />
              <span className="hidden sm:inline">{user?.user_id}</span>
            </div>
            <div className="hidden opacity-75 sm:block">{user?.role === "relationship_manager" ? "Relationship Manager" : "Credit Analyst"}</div>
          </Link>
          <button
            type="button"
